...
export default function UpdateOrder({ idOrder, total_C, total_U, idTable, ubi } : { idOrder : any, total_C : number, total_U : number, idTable? : any, ubi?: any }) {
    ...
    async function HandleSubmit(e : any) {
        ...
        if (ubi === 2) {
            ...
            const url = `${process.env.NEXTAUTH_URL_INTERNAL}print/printComanda/${orderIdAndTableId}`;
        ...
        if (ubi === 1) {
            ...
            const { data } = res;
            const url = `${process.env.NEXTAUTH_URL_INTERNAL}print/printComanda/${data.id}`;
            const windowFeatures = 'noopener,noreferrer';
            window.open(url, '_blank', windowFeatures);
        }
    }
