...
export default function InformationCards({title, data, style1, style2} : {title : string, data : any, style1: string, style2 : string }) {
    ...
    return (
    <motion.article 
    className={`w-[46%] md:w-1/4 bg-white rounded-lg shadow-md p-2 md:p-4 flex justify-between items-center border-b-large ${style1}`}
    initial={{ y: "-100vh" }}
    animate={{ y: 0 }}
    transition={{ duration: 0.5 }}
    >
        <header>
            <h2 className="text-xs font-semibold text-gray-500 uppercase w-full">{title}</h2>
        ...
        <figure className={` ${style2} rounded-full w-6 h-6 md:h-10 md:w-10 flex items-center justify-center`}></figure>
    </motion.article>
    )
}
